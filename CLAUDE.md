...
- **Mémoire long-terme** : deux niveaux. (1) **Fichiers `.md` par utilisateur** (`memory/files.py`, volume `SPOUET_MEMORY_DIR`), lus **à la demande** par l'IA via les built-ins `memory_list`/`memory_read`/`memory_write`/`memory_delete` — seul l'**index** (noms + descriptions) est injecté dans le system prompt (`context.build_extra_system`), pas le contenu. Gérables aussi via la page `/memory`. (2) **Identité key/value épinglée** (`memory/store.py`, `Memory` pinned) injectée à chaque tour pour la persona (prénom, nom de l'IA, ton…), remplie par l'Onboarding. L'ancien recall vectoriel auto-injecté des mémoires non-épinglées a été **remplacé** par les fichiers `.md`.
...
- **Tools built-in** : `orchestrator/builtin_tools.py` déclare et exécute en-process (hors Docker) `web_search`, `show_visual`, `list_macros`, `run_desktop_action`, `run_macro`, `define_macro`, `memory_*` (mémoire .md), `generate_image`, et `spawn_subagent` — sur le modèle de `delegate_to_node`. Dispatch dans `chat_loop._execute_tool_call` (et `chat_loop` builtin branch) avant le lookup des tools Docker.
- **Sous-agents (agentic)** : built-in `spawn_subagent` → l'IA délègue une sous-tâche autonome à une instance fraîche d'elle-même (conversation éphémère `archived=True`, mêmes outils), récupère le résultat, et synthétise. Profondeur bornée par `MAX_SUBAGENT_DEPTH` (contextvar `_SUBAGENT_DEPTH` ; le tool n'est plus exposé au fond). Réutilise `stream_assistant_reply` (import tardif anti-cycle), comme `delegate_to_node`.
- **Tool calls en texte** : garde-fou `orchestrator/tool_salvage.py` — si un modèle écrit son appel d'outil en clair (formats `<|tool_call>…`, `call:NAME{…}`, JSON nu) au lieu du canal structuré (llama-server ne le parse pas), `chat_loop` le récupère en dernier recours et nettoie le contenu affiché. `SPOUET_DISABLE_THINKING` (défaut on) passe `chat_template_kwargs.enable_thinking=false` à llama-server (Qwen3…).
- **Aperçu live de code** : `web/.../Markdown.svelte` ajoute un bouton « Aperçu » sur les blocs `html/svg/xml` → rendu dans une iframe `sandbox` (allow-scripts sans allow-same-origin, isolation totale). PHP/exécution serveur hors scope.
...
| `memory/` | Mémoire long-terme : fichiers `.md` par user lus à la demande (`files.py`) + identité key/value épinglée pour la persona (`store.py`) |
...
- `SPOUET_AGENT_TARGET_COMMIT` (backend) : commit git cible du monorepo pour l'auto-update des node-agents. **Stampé automatiquement par `install.sh`** dans `deploy/.env` (`git rev-parse --short HEAD`) à chaque run. Le heartbeat renvoie `update_available=true` aux agents dont le commit diffère → ils se mettent à jour seuls (`git pull` + redémarrage du service). Vide = pas de signal d'update.
- `SPOUET_AGENT_AUTO_UPDATE` (node-agent, défaut `1`) : mettre à `0`/`false` pour désactiver l'auto-update d'un node (il ignorera le signal `update_available` ; mise à jour alors manuelle via `node-agent/install.sh`).
...
- `SPOUET_MEMORY_DIR` (backend, défaut `/data/memory`, volume backend+worker) : répertoire des fichiers mémoire `.md` par utilisateur (`{memory_dir}/{user_id}/*.md`). `SPOUET_MEMORY_MAX_FILES_PER_USER` (défaut 200) et `SPOUET_MEMORY_MAX_FILE_BYTES` (défaut 64000) plafonnent l'usage. L'IA lit/écrit ces fichiers via les built-ins `memory_*` ; l'index est injecté dans le system prompt.
- `IMAGES` / `IMAGE_MODEL` / `IMAGE_PORT` (installeur node-agent `install.sh`/`install.ps1`) : la génération d'images est **activée par défaut** (`IMAGES=1`) — l'extra `spouet-agent[images]` (torch/diffusers) est installé et le service relancé avec `uv run --extra images`. Désactiver : `IMAGES=0` / `--no-images` (bash) ou `-NoImages` (ps1). `IMAGE_MODEL` est **optionnel** : normalement on choisit/télécharge le modèle depuis la page détail node de l'UI (champ libre repo HF, comme les modèles GGUF). `IMAGE_DEVICE`/`IMAGE_DTYPE` (env node-agent) : vide = auto (device `cuda`/`cpu`, modèle par défaut SDXL-Turbo GPU / SD-Turbo CPU si aucun choisi). Premier pull = téléchargement des poids (cache HF du node).
